    ...
    <div className="w-full">
      <div className="bg-[#5271ff] w-full text-white uppercase font-semibold relative">
        <div className="px-8 flex justify-between items-center relative h-[50px]">
          <div
            ...
            className={` text-3xl flex lg:hidden font-bold h-full w-[50px] cursor-pointer justify-center items-center ${cate_show ? "bg-[#00000026]" : ""} hover:bg-[#00000026] `}
          >
            <IoMdList />
          </div>
          ...
          <div className="flex-wrap hidden lg:flex">
            <Link
              className={`px-6 font-medium py-[13px] hover:bg-[#00000026] ${path === "/" ? "bg-[#00000026]" : ""} `}
              href={"/"}
            >
              {" "}
              Home{" "}
            ...
            {categories.length > 0 &&
              categories.map((c, i) => (
                <Link
                  key={i}
                  className={`px-6 font-medium py-[13px] hover:bg-[#00000026] ${path === c.category ? "bg-[#00000026]" : ""} `}
          ...
          <div className="h-full w-[50px]">
            ...
            <div
              className={`absolute lg:block transition-all text-slate-700 z-20 shadow-lg lg:right-10 top-[50px] w-full lg:w-[300px] right-0 ${show ? "visible" : "invisible"} `}
            >
              <Search />
      ...
      {cate_show && (
        <div className="flex flex-wrap lg:hidden py-2 px-[30px]">
          <Link
            className={`px-4 font-medium py-[5px] ${path === "/" ? "bg-[#00000026]" : ""} `}
            href={"/"}
          >
            {" "}
            Home{" "}
          ...
          {categories.length > 0 &&
            categories.map((c, i) => (
              <Link
                key={i}
                className={`px-4 font-medium py-[5px] ${path === c.category ? "bg-[#00000026]" : ""} `}
                href={`/news/category/${c.category}`}
              >
                {" "}
                {c.category}{" "}
