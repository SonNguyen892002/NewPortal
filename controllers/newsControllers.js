...
class newsControllers {
  ...
  get_categories = async (req, res) => {
    try {
      const categories = await newsModel.aggregate([
        ...
        // Ensure a stable, fixed order (alphabetical) for frontend display
        {
          $sort: { category: 1 },
        },
      ]);
  ...
  news_statistics = async (req, res) => {
    ...
    try {
      ...
      const activeNews = await newsModel.countDocuments({
        ...filter,
        status: "active",
      });
      const deactiveNews = await newsModel.countDocuments({
        ...filter,
        status: "deactive",
      });
